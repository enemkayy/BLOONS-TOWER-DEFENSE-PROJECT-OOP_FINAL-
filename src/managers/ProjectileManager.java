...
public class ProjectileManager {
	...
	private void importImgs() {
		...
		BufferedImage shuriken = LoadSave.getShuriken();
		proj_imgs = new BufferedImage[3];

		proj_imgs[0] = LoadSave.getDart();
		proj_imgs[1] = shuriken.getSubimage(1 * 32, 0, 32, 32);
		proj_imgs[2] = atlas.getSubimage(9 * 32, 32, 32, 32);
	...
	private boolean isProjHittingEnemy(Projectile p) {
		for (Enemy e : playing.getEnemyManager().getEnemies()) {
			if (e.getBounds().contains(p.getPos())) {
				e.hurt(p.getDmg());
				if (p.getProjectileType() == SHURIKEN)
					e.slow();
				return true;
			}
		}
	...
	public void draw(Graphics g) {
		...
		for (Projectile p : projectiles)
			if (p.isActive()) {
				if (p.getProjectileType() == DART || p.getProjectileType() == SHURIKEN) {
